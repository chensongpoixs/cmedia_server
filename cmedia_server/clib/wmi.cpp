...
{
	TCHAR * path = (TCHAR *)L"ExecutablePath";
    return GetProcStringProperty(pid, path, value, MAX_PATH);
}

HRESULT WMI::GetProcCommandLine(DWORD pid, TCHAR *value)
{
	TCHAR * path = (TCHAR *)L"CommandLine";
    return GetProcStringProperty(pid, path, value, SIGAR_CMDLINE_MAX);
}
